...
const ChatRequestItem = ({
  ...
  const handleAcceptRequest = async () => {
    ...
    const url = `https://house-rental-oxbl.onrender.com/accept-chat-request/${chatId}`;
    const headers = {
  ...
  const handleRejectRequest = async () => {
    const token = Cookies.get("jwt_token");
    const url = `https://house-rental-oxbl.onrender.com/delete-chat-request/${chatId}`; // Ensure this is correct
    const headers = {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    };
